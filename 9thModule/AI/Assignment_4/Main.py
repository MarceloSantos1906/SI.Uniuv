...
class App:
    ...
    def rgb(self):
        self.root.withdraw()
        new_window = tk.Toplevel(self.root)
        new_window.title("RGB Trainer")

        app = RgbApp(new_window)
...
class RgbApp:
    ...
    def addColorField(self, frame):
        ...
        btn_color = ttk.Button(
            ...
            command=lambda e=entry_g: self.pick_color_from_image(self.char1_pictures_path if frame in self.id_entries_per_frame else self.char2_pictures_path, e)
        )
        btn_color.grid(row=row, column=2)
    ...
    def count_pixels_matching_colors(self, image_path, target_rgbs, margin=0.1):
        img = cv2.imread(image_path)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (100, 100))
        ...
        for target in target_rgbs:
            lower_bound = np.array([max(0, c * (1 - margin)) for c in target], dtype=np.uint8)
            upper_bound = np.array([min(255, c * (1 + margin)) for c in target], dtype=np.uint8)
    ...
    def test_model(self):
        ...
        feature_vector = self.count_pixels_matching_colors(self.tset_image, self.target_rgbs, margin=0.1)
        feature_vector = np.array(feature_vector).reshape(1, -1)
        ...
        if predicted_label == 0:
            name = self.char1_name.get()
        else:
            name = self.char2_name.get()

        self.result.configure(text=name)
    ...
    def match_rgb_with_targets(self, image_path, target_rgbs, n_clusters=5):
        img = cv2.imread(image_path)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (100, 100))
        ...
        _centers = kmeans._centers_.astype(int)

        matches = []
        for target_rgb in target_rgbs:
            distances = [
                self.color_distance(target_rgb, _rgb)
                for _rgb in _centers
            ]
            closest_rgb = _centers[np.argmin(distances)]
            matches.append(tuple(closest_rgb))
    ...
    def generate_dataset_from_targets(self, base_dir, headers, target_rgbs, output_csv, character_names):
        with open(output_csv, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(headers)
            ...
            for images_path in base_dir:
                ...
                for filename in os.listdir(images_path):
                    ...
                    if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                        self.status.configure(text=f"Processing {path}")
                        try:
                            pixel_counts = self.count_pixels_matching_colors(
                                path, target_rgbs, margin=0.1
                            )

                            index = base_dir.index(images_path)
                            label = character_names[index]

                            row = pixel_counts + [label]
                            writer.writerow(row)
    ...
    def pick_color_from_image(self, folder_path, entry_widget):
        if not folder_path:
            messagebox.showerror("Error", "Pasta não selecionada.")
            return

        images = [f for f in os.listdir(folder_path) if f.lower().endswith((".png", ".jpg", ".jpeg"))]
        if not images:
            messagebox.showerror("Error", "Nenhuma imagem encontrada na pasta.")
            return
        ...
        pick_window = tk.Toplevel(self.root)
        pick_window.title("Escolha uma cor")

        img_tk = ImageTk.PhotoImage(img)

        label = tk.Label(pick_window, image=img_tk)
        label.image = img_tk
        label.pack()
        ...
        def click_event(event):
            ...
            entry_widget.insert(0, f"({r}, {g}, {b})")
            pick_window.destroy()

        label.bind("<Button-1>", click_event)


class ConvolutionalNeuralNetworkApp:
    def __init__(self, root):
        self.root = root
        ...
        root.title("Convolutional Neural Network Trainer")
        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        root.configure(width=screen_width, height=screen_height)
        if platform.system() == "Windows":
        ...
        self.main_frame = ttk.Frame(
            self.canvas, style="Custom.TFrame", width=screen_width
        )
        self.canvas.create_window((0, 0), window=self.main_frame, anchor="nw")
    ...
    def add_training_parameters(self, frame):
        ...
        ttk.Button(
            ...
            text="Treinar",
            width=20,
            command=lambda: self.start_task(self.start_training),
        ).grid(row=8, column=0, columnspan=3, pady=5, sticky="nsew")
        ttk.Button(
            frame,
            text="Carregar Modelo Existente",
    ...
    def start_training(self):
        X, y = self.treat_images(img_size=(64, 64))
        X = X / 255.0
        ...
        self.rede_neural = self.build_cnn_model(input_shape=(64, 64, 3))

        early_stopping = EarlyStopping(
            monitor="val_loss", patience=5, restore_best_weights=True
        )

        self.rede_neural.fit(
            datagen.flow(X_train, y_train, batch_size=int(self.batch_size.get())),
    ...
    def treat_images(self, img_size=(64, 64)):
        folders = [self.char1_pictures_path, self.char2_pictures_path]
        labels = [0, 1]
        images = []
        targets = []
    ...
    def test_model(self):
        ...
        img = cv2.imread(self.tset_image)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (64, 64))
        img = img / 255.0
        img = np.expand_dims(img, axis=0)

        prediction = self.rede_neural.predict(img)
        print(f"Raw prediction: {prediction}")

        if prediction >= 0.5:
            label = self.char2_name.get()
        else:
            label = self.char1_name.get()

        self.result.configure(text=label)
...
class ConvolutionalNeuralNetworkMultipleCharApp:
    def __init__(self, root):
        self.root = root
        ...
        root.title("Convolutional Neural Network Trainer Multiple Charaters")
        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        root.configure(width=screen_width, height=screen_height)
        if platform.system() == "Windows":
    ...
    def open_cnn_window(self):
        ...
        self.characters_frame = ttk.Frame(
            self.main_frame, style="Custom.TFrame", padding=10
        )
        self.characters_frame.pack(
            padx=10, pady=10, side="left", expand=True, fill="both"
        )

        ttk.Button(
            self.characters_frame,
            text="Add charater",
            command=lambda: self.add_character_input(),
    ...
    def get_next_char_frame_position(self):
        if not self.char_frame_pos:
            return (1, 0)

        last_row, last_col = self.char_frame_pos[-1]
        if last_col < 1:
            return (last_row, last_col + 1)
        else:
            return (last_row + 1, 0)

    def add_character_input(self):
        row, col = self.get_next_char_frame_position()

        char_frame = ttk.Frame(self.characters_frame, style="Custom.TFrame", padding=10)
        char_frame.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")

        self.char_frame_pos.append((row, col))
        self.char_number += 1

        title = ttk.Label(
            char_frame, text=f"Personagem {self.char_number}", font=("Arial", 14)
        )
        title.grid(row=0, column=0, columnspan=3, pady=(10, 0))

        self.char_name[self.char_number] = None
        self.char_pictures_path[self.char_number] = None

        labelNameChar = ttk.Label(char_frame, text="Nome do personagem:")
        labelNameChar.grid(row=1, column=0, sticky="w")
        char_name_entry = ttk.Entry(char_frame)
        char_name_entry.grid(row=1, column=1)

        labelImages = ttk.Label(char_frame, text="Imagens do personagem:")
        labelImages.grid(row=2, column=0, sticky="w")
        entry_path = ttk.Entry(char_frame)
        entry_path.grid(row=2, column=1)

        ttk.Button(
            char_frame,
            text="Select folder",
            command=lambda: self.select_picture_folder(entry_path),
        ).grid(row=2, column=2)

        self.char_name[self.char_number] = char_name_entry
        self.char_pictures_path[self.char_number] = entry_path
    ...
    def add_training_parameters(self, frame):
        ...
        self.input_shape = tk.IntVar()
        self.input_shape_label = tk.StringVar(value="0.00")
        self.input_shape_slider = ttk.Scale(
            frame, from_=0, to=100, orient="horizontal", variable=self.input_shape
        )
        self.input_shape_slider.grid(row=1, column=1)
        ttk.Label(frame, textvariable=self.input_shape_label, width=5).grid(
            row=1, column=2
        )
        ...
        self.units = tk.IntVar()
        self.units_label = tk.StringVar(value="0.00")
        self.units_slider = ttk.Scale(
            frame, from_=0, to=100, orient="horizontal", variable=self.units
        )
        self.units_slider.grid(row=2, column=1)
        ttk.Label(frame, textvariable=self.units_label, width=5).grid(row=2, column=2)

        self.layers = tk.IntVar()
        self.layers_label = tk.StringVar(value="0.00")
        self.layers_slider = ttk.Scale(
            frame, from_=0, to=100, orient="horizontal", variable=self.layers
        )
        self.layers_slider.grid(row=3, column=1)
        ttk.Label(frame, textvariable=self.layers_label, width=5).grid(row=3, column=2)

        self.bias = tk.IntVar()
        self.bias_label = tk.StringVar(value="0.00")
        self.bias_slider = ttk.Scale(frame, from_=0, to=1, orient="horizontal", variable=self.bias)
        self.bias_slider.grid(row=4, column=1)
        ttk.Label(frame, textvariable=self.bias_label, width=5).grid(row=4, column=2)
        ...
        self.test_size_slider.grid(row=7, column=1)
        ttk.Label(frame, textvariable=self.test_size_label, width=5).grid(
            row=7, column=2
        )

        self.input_shape_slider.bind("<MouseWheel>", lambda e: self.scroll_scale(e, self.input_shape))
        self.units_slider.bind("<MouseWheel>", lambda e: self.scroll_scale(e, self.units))
        self.layers_slider.bind("<MouseWheel>", lambda e: self.scroll_scale(e, self.layers))
        self.bias_slider.bind("<MouseWheel>", lambda e: self.scroll_scale(e, self.bias))
        self.test_size_slider.bind("<MouseWheel>", lambda e: self.scroll_scale(e, self.test_size))
        ...
        ttk.Button(
            frame, text="Salvar parametros", width=20, command=lambda: self.save_params()
        ).grid(row=11, column=0, columnspan=3, pady=5, sticky="nsew")
        ttk.Button(
            frame, text="Carregar parametros", width=20, command=lambda: self.load_params()
        ).grid(row=12, column=0, columnspan=3, pady=5, sticky="nsew")

    def scroll_scale(self, event, variable):
        scroll_step_test_size = 0.01
        scroll_step = 1
        if variable == self.test_size:
            delta = scroll_step_test_size if event.delta > 0 else scroll_step_test_size
        else:
            delta = scroll_step if event.delta > 0 else -scroll_step
    ...
    def select_picture_folder(self, entry_path):
        folder = filedialog.askdirectory(initialdir=os.path.expanduser("~/Downloads"))
        try:
            entry_path.delete(0, tk.END)
            entry_path.insert(0, folder)
    ...
    def treat_images(self, img_size=(64, 64)):
        ...
        labels = []
        i = 0
        for key, entry in self.char_pictures_path.items():
            folders.append(entry.get())
            labels.append(i)
            i+=1
        ...
        for folder, label in zip(folders, labels):
            for filename in os.listdir(folder):
                file_path = os.path.join(folder, filename)
                if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                    self.status.configure(text=f'Formating file: {file_path}')
                    img = cv2.imread(file_path)
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                    img = cv2.resize(img, img_size)
                    images.append(img)
    ...
    def test_model(self):
        ...
        img = cv2.resize(img, (64, 64))
        img = img / 255.0
        feature_vector = np.expand_dims(img, axis=0)

        prediction = self.rede_neural.predict(feature_vector)
        predicted_class_index = int(np.argmax(prediction, axis=1)[0])

        character_name = self.char_name[predicted_class_index + 1].get()
    ...
    def save_model(self):
        ...
        if path:
            try:
                ...
                i = 0
                for key, entry in self.char_pictures_path.items():
                    metadata[f"char{i}_path"] = entry.get()
                    i += 1
                i = 0
                for key, entry in self.char_name.items():
                    metadata[f"char{i}_name"] = entry.get()
                    i += 1
                metadata.update({
                    "input_shape": self.input_shape.get(),
                    "units": self.units.get(),
    ...
    def save_params(self):
        ...
        if path:
            try:
                ...
                with open(metadata_path, "w") as f:
                    json.dump(metadata, f)
                self.status.configure(text=f"Metadata saved to: {path}")
            except Exception as e:
                messagebox.showerror("Error", f"Error saving file: {e}")
    ...
    def load_model(self):
        ...
        try:
            ...
            if os.path.exists(json_path):
                ...
                i = 0
                while True:
                    char_path_key = f"char{i}_path"
                    char_name_key = f"char{i}_name"
                    if char_path_key in metadata and char_name_key in metadata:
                        loaded_chars.append((metadata[char_path_key], metadata[char_name_key]))
                        i += 1
                    else:
                        break
                ...
                for path_val, name_val in loaded_chars:
                    self.add_character_input_from_data(path_val, name_val)

                self.input_shape.set(metadata.get("input_shape", 0))
                self.units.set(metadata.get("units", 0))
                self.layers.set(metadata.get("layers", 0))
                self.bias.set(metadata.get("bias", 0))
                self.test_size.set(metadata.get("test_size", 0.2))

                self.epochs.delete(0, tk.END)
                self.epochs.insert(0, metadata.get("epochs", ""))
                self.batch_size.delete(0, tk.END)
                self.batch_size.insert(0, metadata.get("batch_size", ""))

                self.status.configure(text="Model and metadata loaded successfully.")
    ...
    def load_params(self):
        path = filedialog.askopenfilename(
            initialdir="./Models", filetypes=[["", "*.json"]]
        )
        if not path:
            return
        try:
            if os.path.exists(path):
                ...
                messagebox.showerror("Error", f"Metadata não encontrados")
                return

        except Exception as e:
            messagebox.showerror("Error", f"Failed to load model: {e}")

    def add_character_input_from_data(self, path_val, name_val):
        row, col = self.get_next_char_frame_position()
        char_frame = ttk.Frame(self.characters_frame, style="Custom.TFrame", padding=10)
        char_frame.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")
        self.char_frame_pos.append((row, col))
        ...
        ttk.Label(char_frame, text=f"Personagem {self.char_number}", font=("Arial", 14)).grid(row=0, column=0, columnspan=3, pady=(10, 0))

        labelNameChar = ttk.Label(char_frame, text="Nome do personagem:")
        labelNameChar.grid(row=1, column=0, sticky="w")
        char_name_entry = ttk.Entry(char_frame)
        char_name_entry.grid(row=1, column=1)
        char_name_entry.insert(0, name_val)

        labelImages = ttk.Label(char_frame, text="Imagens do personagem:")
        labelImages.grid(row=2, column=0, sticky="w")
        entry_path = ttk.Entry(char_frame)
        entry_path.grid(row=2, column=1)
        entry_path.insert(0, path_val)
    ...
    def tset(self):
        ...
        print(type(self.char_name))
        for key, entry_widget in self.char_name.items():
            print(key)
            print(type(key))
            print(entry_widget)
            print(type(entry_widget))
